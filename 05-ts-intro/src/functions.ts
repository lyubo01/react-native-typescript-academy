import {PointImpl} from './interface.js'
...
function firstElement1<Type>(arr: Type[]):Type { //good
    return arr[0];
  }
  ...
  function filter1<Type>(arr: Type[], func: (arg: Type) => boolean): Type[] { //good
    return arr.filter(func);
  }
  function filter2<Type, Func extends (arg: Type) => boolean>( //bad
    arr: Type[],
    func: Func
  ): Type[] {
    return arr.filter(func);
